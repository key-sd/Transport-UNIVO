...
// validaciones
$errores = [];
if ($ruta_id === 0) $errores[] = 'ID de ruta inválido.';
...
// verificar que no exista ya esa combinación origen-destino en otra ruta distinta
$stmtDup = $conn->prepare("SELECT id FROM rutas WHERE id_sede_origen = ? AND id_sede_destino = ? AND id != ? LIMIT 1");
$stmtDup->bind_param('iii', $origen, $destino, $ruta_id);
$stmtDup->execute();
...
try {
    $stmt = $conn->prepare("UPDATE rutas SET id_sede_origen = ?, id_sede_destino = ? WHERE id = ?");
    $stmt->bind_param('iii', $origen, $destino, $ruta_id);
    $stmt->execute();
    $stmt->close();
    ...
    error_log('[editar_ruta] ' . $e->getMessage());
